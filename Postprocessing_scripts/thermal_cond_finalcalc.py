# -*- coding: utf-8 -*-
"""thermal_cond_finalcalc.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1neJr4NJl4-CxlX5sQ-EgdPT7mKwveOV8
"""
  ...
  for i in range(len(test_points_index)):
    ...
    for j in range(len(running_average_data[0,:])):
      ...
      for k in check_range:
        if relative_abs_error(running_average_data[check_index,j], running_average_data[(check_index+k),j]) > 0.002:
          break
      else:
        continue
      break
...
ext_lo = np.loadtxt("ext_final_temp_profile_DIR_INDEX_thermalcond.txt")
...
lammps_input = pd.read_table("thermalsim.DIR_INDEX_reshape_HFD.txt", delimiter=" ")
...
###### Check value of dE_lo and make sure it is the correct one #######################
...
z_interval = np.linspace(0.025, 0.975, 20) #linsapce when you know the exact start, stop and number
...
np.savetxt("kappa_value_DIR_INDEX.txt", kappa_value)
np.savetxt("kappa_value_percenterror_DIR_INDEX.txt", kappa_value_percenterror)
np.savetxt("kappa_value_abserror_DIR_INDEX.txt", kappa_value_abserror)
